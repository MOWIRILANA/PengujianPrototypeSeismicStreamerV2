...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler("modbus_master_graph.log")],
)
...
# Buffer to hold data for 1 second
buffer = {}
...
async def modbus_main(master):
    ...
    global buffer, data_count, start_time, register_counts
    try:
        while True:
            for slave_id in master.slave_ids:
                ...
                if voltages:
                    ...
                    with lock:
                        ...
                        # Buffer data every second
                        if slave_id not in buffer:
                            buffer[slave_id] = {"x": [], "A0": [], "A1": [], "A2": [], "A3": []}

                        buffer[slave_id]["x"].append(x)
                        for i, key in enumerate(["A0", "A1", "A2", "A3"]):
                            buffer[slave_id][key].append(voltages[i])
            ...
            if current_time - start_time >= 1:
                with lock:
                    for slave_id in slave_ids:
                        # Print and process the buffer data
                        if slave_id in buffer:
                            print(f"Buffer for Slave {slave_id}: {len(buffer[slave_id]['x'])} data points")
            ...
            await asyncio.sleep(0.0001)  # Polling interval
    except Exception as e:
...
if __name__ == "__main__":
    slave_ids = [1]  # Set slave IDs here
