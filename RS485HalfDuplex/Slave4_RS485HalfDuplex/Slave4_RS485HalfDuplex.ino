...
void setup() {
  ...
  mb4.begin(&Serial2, DE_RE_PIN);  // Pass DE_RE pin to library
  mb4.slave(4);                    // Set Modbus slave ID to 1

  // Initialize Modbus holding registers untuk setiap channel
  for (int i = 0; i < 4; i++) {
    mb4.addHreg(i);  // Register index matches the channel number
