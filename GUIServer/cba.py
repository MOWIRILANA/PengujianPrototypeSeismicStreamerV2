...
latest_data = [None] * NUM_REGISTERS  # List untuk menyimpan data dari 4 register
        ...
        try:
            while True:
                ...
                if response.isError():
                    ...
                    with data_lock:
                        latest_data = response.registers  # Simpan semua nilai register ke latest_data
                        data_buffer.append(latest_data.copy())  # Tambahkan data terbaru ke buffer
                print(f"Data Modbus: {latest_data}")
                time.sleep(0.01)  # Baca data setiap 1 ms
